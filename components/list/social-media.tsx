...
const ListSocialMedia = (props: TProps) => {
  ...
  return (
    <div className='w-full'>
        ...
        <ul className='mb-3 flex flex-col gap-2 list-data'>
            {
                socialMedias.map((data:TSocialMedia,i:number)=>(
                    <Li key={i} id={data.id} type="link" link={data.link} name={data.name} onRemove={handleDeleteData} onEdit={handleEditData} setShowFormEdit={setShowForm}/>
                ))
            }
        </ul>
        {

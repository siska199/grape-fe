...
const Modal = ({children, label, onClose, onSave}: Props) => {
  return (
    <article className='fixed top-0 left-0  bg-white/20 flex justify-center items-center w-full h-full'>
        <section className='font-thin w-[40rem]  p-4 min-h-[20rem] border relative border-dashed bg-cd900 shadow-md shadow-cd800'>
            ...
            <form action=" 
            "className='flex flex-col gap-4 px-4 max-h-[30rem] overflow-x-hidden overflow-y-scroll '>
              {children}
            </form>
            <div className=' sticky bottom-0 bg-cd900 pt-4 flex'>
              <Button label={"Save"} customeStyle='bg-cd700 py-2 w-[7rem] ml-auto' onClick={onSave}/>
            </div>
        </section>
    </article>
  )
